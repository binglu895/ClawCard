...
export const GENERATE_SHOP_ITEMS = (year: number, currentEquipment: Record<string, Joker | null>) => {
    ...
    const isMaxLevel = (baseId: string) => {
        ...
        if (!equipped || !equipped.level) return false;
        const tier = GET_ITEM_TIER(baseId);
        const maxLevel = tier === 5 ? 5 : 3; // Tier 5 max is 5, others max is 3
        return equipped.level >= maxLevel;
    };
...
export const GET_JOKER_STATS = (joker: Joker, state: GameState, selectedCards: CardData[], currentHand: PokerHand): { tao: number, mult: number, xMult: number } => {
    const tier = GET_ITEM_TIER(joker.id);
    const maxLvl = tier === 5 ? 5 : 3;
    const lvl = Math.min(joker.level || 1, maxLvl);
    let tao = 0;
    let mult = 0;
    let xMult = 1.0;
